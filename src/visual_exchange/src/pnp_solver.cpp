...
PNPSolver::PNPSolver(double fx, double fy, double u0, double v0, double k_1,
                     double k_2, double p_1, double p_2, double k_3) {
  //初始化输出矩阵
  vector<double> rv(3), tv(3);
  cv::Mat rvec(rv), tvec(tv);
...
int PNPSolver::Solve(METHOD method) {
  ...
  } else if (method == METHOD::CV_SQPNP) {
    if (Points3D.size() < 3)
      return -2;
  } else {
    if (Points3D.size() < 4) {
  ...
  ////TODO::检验是否是共面的四点
  // if ((method == METHOD::CV_ITERATIVE || method == METHOD::CV_EPNP) &&
  // Points2D.size() == 4)
  //{
  ...
  /*******************解决PNP问题*********************/
  //有三种方法求解
  if (!solvePnP(Points3D, Points2D, camera_matrix, distortion_coefficients,
                rvec, tvec, false, method)) {
    // printf("Error Pnp\n");
    return -1;
  }
  //实测迭代法似乎只能用共面特征点求位置
  // solvePnP(Points3D, Points2D, camera_matrix, distortion_coefficients,
  // rvec, tvec, false, CV_ITERATIVE); //实测迭代法似乎只能用共面特征点求位置
  // solvePnP(Points3D, Points2D, camera_matrix, distortion_coefficients,
  // rvec, tvec, false, CV_P3P);      //Gao的方法可以使用任意四个特征点
  // solvePnP(Points3D, Points2D, camera_matrix, distortion_coefficients,
...
//输出为点投影到图像上的图像坐标集合
vector<cv::Point2f>
PNPSolver::WordFrame2ImageFrame(vector<cv::Point3f> WorldPoints) {
  vector<cv::Point2f> projectedPoints;
  cv::projectPoints(WorldPoints, rvec, tvec, camera_matrix,
...
//   p4psolver.SetCameraMatrix(2783.51380399845, 2795.03600410101,
//                             1932.22472201653, 1153.49477364402);
//   //设置畸变参数
//   p4psolver.SetDistortionCoefficients(0.0407902477934583, -0.547170508860230,
//   0,
//                                       0, 1.72889744087393);
//   //设置特征点的世界坐标
//   p4psolver.Points3D.push_back(
//       cv::Point3f(4825, -1930, 400)); // P1三维坐标的单位是毫米
//   p4psolver.Points3D.push_back(cv::Point3f(5225, -1930, 400)); // P2
//   p4psolver.Points3D.push_back(cv::Point3f(5225, -2230, 400)); // P3
//   p4psolver.Points3D.push_back(cv::PoinTheta_W2Ct3f(4825, -2230, 400)); // P4

//   cout << "test2:特征点世界坐标 = " << endl << p4psolver.Points3D << endl;
//   //设置特征点的图像坐标
//   p4psolver.Points2D.push_back(cv::Point2f(290, 1819));   // P1
//   p4psolver.Points2D.push_back(cv::Point2f(366, 1733));   // P2
//   p4psolver.Points2D.push_back(cv::Point2f(495, 1732));   // P3
//   p4psolver.Points2D.push_back(cv::Point2f(427., 1819.)); // P4

//   cout << "test2:图中特征点坐标 = " << endl << p4psolver.Points2D << endl;

//   if (p4psolver.Solve(PNPSolver::METHOD::CV_P3P) == 0)
//     cout << "test2:CV_P3P方法:  相机位姿→"
//          << "Oc坐标=" << p4psolver.Position_OcInW
//          << "    相机旋转=" << p4psolver.Theta_W2C << endl;
//   if (p4psolver.Solve(PNPSolver::METHOD::CV_ITERATIVE) == 0)
//     cout << "test2:CV_ITERATIVE方法:    相机位姿→"
//          << "Oc坐标=" << p4psolver.Position_OcInW
//          << "    相机旋转=" << p4psolver.Theta_W2C << endl;
//   if (p4psolver.Solve(PNPSolver::METHOD::CV_EPNP) == 0)
//     cout << "test2:CV_EPNP方法: 相机位姿→"
//          << "Oc坐标=" << p4psolver.Position_OcInW
//          << "    相机旋转=" << p4psolver.Theta_W2C << endl;
