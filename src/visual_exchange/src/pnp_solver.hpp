...
public:
  PNPSolver();
  //带参数初始化
  PNPSolver(double fx, double fy, double u0, double v0, double k_1, double k_2,
            double p_1, double p_2, double k_3);
  ~PNPSolver();
  ...
  //输出参数：CV_ITERATIVE，CV_P3P（默认），CV_EPNP，具体参见Opencv
  // documentation. 实测
  ...
  //设置畸变系数矩阵
  void SetDistortionCoefficients(double k_1, double k_2, double p_1, double p_2,
                                 double k_3) {
  ...
  /********************公有静态方法*********************/
  //点绕任意向量旋转，右手系
  static cv::Point3f RotateByVector(double old_x, double old_y, double old_z,
                                    double vx, double vy, double vz,
                                    double theta) {
    double r = theta * CV_PI / 180;
    double c = cos(r);
    double s = sin(r);
    double new_x = (vx * vx * (1 - c) + c) * old_x +
                   (vx * vy * (1 - c) - vz * s) * old_y +
                   (vx * vz * (1 - c) + vy * s) * old_z;
    double new_y = (vy * vx * (1 - c) + vz * s) * old_x +
                   (vy * vy * (1 - c) + c) * old_y +
                   (vy * vz * (1 - c) - vx * s) * old_z;
    double new_z = (vx * vz * (1 - c) - vy * s) * old_x +
                   (vy * vz * (1 - c) + vx * s) * old_y +
                   (vz * vz * (1 - c) + c) * old_z;
  ...
  // outx outy为旋转后的结果坐标
  static void CodeRotateByZ(double x, double y, double thetaz, double &outx,
                            double &outy) {
  ...
  // outx outz为旋转后的结果坐标
  static void CodeRotateByY(double x, double z, double thetay, double &outx,
                            double &outz) {
    double x1 = x;
    double z1 = z;
  ...
  // outy outz为旋转后的结果坐标
  static void CodeRotateByX(double y, double z, double thetax, double &outy,
                            double &outz) {
